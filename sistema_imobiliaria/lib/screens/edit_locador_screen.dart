...
class EditLocadorScreen extends StatefulWidget {
  final Map<String, dynamic> locador;

  const EditLocadorScreen({super.key, required this.locador});

  @override
  State<EditLocadorScreen> createState() => _EditLocadorScreenState();
}

class _EditLocadorScreenState extends State<EditLocadorScreen> {
  ...
  void _populateFields() {
    _nameController.text = widget.locador['nome'] ?? '';
    _cpfController.text = widget.locador['cpf'] ?? '';
    _rgController.text = widget.locador['rg'] ?? '';
    _telefoneController.text = widget.locador['telefone'] ?? '';
    _emailController.text = widget.locador['email'] ?? '';
    _profissaoController.text = widget.locador['profissao'] ?? '';
    _rendaController.text = widget.locador['renda'] ?? '';
    _cnhController.text = widget.locador['cnh'] ?? '';
    _estadoCivilController.text = widget.locador['estado_civil'] ?? '';
    _dataNascimentoController.text = widget.locador['data_nascimento'] ?? '';
    _referenciaController.text = widget.locador['referencia'] ?? '';
  }
  ...
  Future<void> _updateLocador() async {
    ...
    try {
      ...
      await DatabaseService.updateLocador(widget.locador['id'].toString(), locadorData);
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Text('Locador atualizado com sucesso!'),
              ],
            ),
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('Erro ao atualizar locador: ${e.toString()}'),
              ],
            ),
  ...
  Widget _buildSaveButton() {
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: _isLoading ? null : _updateLocador,
          borderRadius: BorderRadius.circular(16),
